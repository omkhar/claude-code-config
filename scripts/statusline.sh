...
sanitize_display() {
	printf '%s' "$1" | LC_ALL=C tr -d '\000-\010\013\014\016-\037\177' | tr '\n\r\t' '   '
}
...
current_dir=$(sanitize_display "$current_dir")
model_name=$(sanitize_display "$model_name")
folder_name=$(sanitize_display "$folder_name")
git_branch=$(sanitize_display "${git_branch:-}")
